...
def load_lora(pipe, lora_dir):
    ...
    assert os.path.exists(lora_path), f"{lora_path} not found"

    pipe.load_lora_weights(lora_path,weight_name="adapter_model.safetensors",adapter_name="lora1")
...
def apply_lora(pipe, lora_path: Optional[str]):
    """统一管理 LoRA 加载"""
    if lora_path:
        logging.info(f"Loading LoRA from {lora_path}")
        load_lora(pipe, args.lora_path)
...
if __name__ == "__main__":
    ...
    trajs = read_list(os.path.join(args.data_path, "trajectories.txt"))
    ...
    for img_name, prompt, traj_name,flow_name in zip(images, prompts, trajs,flows):
        ...
        traj_path = os.path.join(args.data_path, "trajectories", clean_name)
        ...
        traj = Image.open(traj_path).convert("RGB")  # 单通道
        traj = TF.to_tensor(traj)
        traj = traj * 2.0 - 1.0
        traj = traj.to(device=device,dtype=pipe.transformer.dtype)
