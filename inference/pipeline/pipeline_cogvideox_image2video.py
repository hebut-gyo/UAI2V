...
class CogVideoXImageToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def check_inputs(
        ...
        if len(camera) !=6:
            raise ValueError(
                "`camera` must have 6 dimensions , but"
                f" got: {len(camera)} "
            )
    ...
    def __call__(
        ...
        # TODO 嵌入相机运动
        cam_emb = self.transformer.cam_mlp(camera_motion)
        motion_token = cam_emb.unsqueeze(1)
        prompt_embeds[:, 0] = prompt_embeds[:, 0] + motion_token
