...
from ...datasets.i2v_dataset import logger
...
class CogVideoXI2VLoraTrainer(Trainer):
    ...
    def load_components(self) -> Dict[str, Any]:
        ...
        #     load_in_4bit=True,  # 启用 4-bit 权重
        #     bnb_4bit_compute_dtype=torch.float16,  # 计算用 BF16
        #     bnb_4bit_use_double_quant=True,  # 双量化（可选）
        #     bnb_4bit_quant_type="nf4"  # NF4 类型（精度最高）
        #  )
        ...
        components.transformer = CogVideoXTransformer3DModel.from_pretrained(
            ...
            motion_block_range=tuple(self.args.motion_block_range)
            if self.args.motion_block_range is not None else None
        )

        # 关键：实例化所有 meta tensor 参数
        def init_meta_tensors(module):
            for name, param in module.named_parameters(recurse=False):
                if param is not None and param.device.type == 'meta':
                    # 创建实际参数
                    new_param = torch.nn.Parameter(
                        torch.empty_like(param, device='cpu', dtype=torch.float16)
                    )
                    # 初始化（可选）
                    if 'bias' in name:
            ...
            for name, buf in module.named_buffers(recurse=False):
                if buf is not None and buf.device.type == 'meta':
                    new_buf = torch.empty_like(buf, device='cpu', dtype=torch.float16)
                    setattr(module, name, new_buf)
        # 递归应用到所有子模块
        components.transformer.apply(init_meta_tensors)
    ...
    def compute_loss(self, batch, scale, global_step) -> torch.Tensor:
        ...
        # if trajs_latents is not None:
        #     trajs_latents = trajs_latents.permute(0, 2, 1, 3, 4)
        ...
        latent_padding = latent.new_zeros(padding_shape)
    ...
    def validation_step(
        ...
        """
        prompt, image, video, traj, flow = eval_data["prompt_embedding"], eval_data["image"], eval_data["video"], eval_data["traj"], eval_data["flow"]
